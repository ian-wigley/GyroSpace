...
public:
	...
	void Paused(bool _pause)
	{
		...
		if (_pause)
		{
			m_timePaused = m_currentTime;
		}
		else
		{
			// Duration that the timer is paused.
			m_paused += static_cast<float>(	static_cast<double>(m_currentTime.QuadPart - m_timePaused.QuadPart) / static_cast<double>(m_frequency.QuadPart));
		}
	}
